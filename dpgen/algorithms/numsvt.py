...
def unpack_alignments(alignments: np.ndarray):
    proof, holes = alignments[:10], alignments[10:]
...
def convert_alignment_str(alignments):
    ...
    return f"alignments: eta1: {proof[0]}, eta2: Omega ? {eta2_true_dist} : {eta2_false_dist}, eta3: {eta3_dist}", f"eta1: {holes[0]} | eta2: {eta2} | eta3: {eta3}"
...
def svt(all_inputs, alignments):
    ...
    while i < LENGTH and count < c:
        if q[i] + noise[i + 1] >= T_bar:
            ...
            # output q[i] + noise[2 * i + 1]
            eta3_dist = (proof[7] + proof[8] * dist_T_bar + proof[9] * dq[i])
            failures += my_assert(dq[i] + eta3_dist == 0)
            cost += np.abs(eta3_dist) * EPSILON / (holes[3] + holes[4] * c)
        else:
    ...
    return cost, failures, (np.square(holes[0] / EPSILON) + np.square((holes[1] + holes[2] * c) / EPSILON) + np.square(
        (holes[3] + holes[4] * c) / EPSILON)) / (
                       np.square(9 / EPSILON) + np.square((9 + 9 * c) / EPSILON) + np.square((9 + 9 * c) / EPSILON))
...
def find_inputs(all_inputs, alignments):
    ...
    for i in numba.prange(all_inputs.shape[0]):
        ...
        if N > int(
                LENGTH / 5):  # or np.linalg.norm(dq, ord=1) < 0.5 * LENGTH: # NOTICE: this constrains norm(dq) > 0.8 * LENGTH
            results[i] = 1000 * N
        else:
...
def main():
    ...
    min_bounds = (np.array(
        [-10 for _ in range(LENGTH)] + [-1 for _ in range(LENGTH)] + [-10 for _ in range(2 * LENGTH + 1)] + [-10, 1]))
    max_bounds = (np.array(
        [10 for _ in range(LENGTH)] + [1 for _ in range(LENGTH)] + [10 for _ in range(2 * LENGTH + 1)] + [10, LENGTH]))
    bounds = (min_bounds, max_bounds)

    align_min_bounds = [-5 for _ in range(10)] + [0, 0, 0, 0, 0]
    align_max_bounds = [5 for _ in range(10)] + [5, 10, 10, 5, 10]
    alignment_bounds = (align_min_bounds, align_max_bounds)
    # q, dq, noise, T, N
    alignment = np.array([0 for _ in range(10)] + [1, 1, 0, 1, 0])
    # alignment = np.array([1, 1, 0, -1, 0, 0, 0, 0, 0, -1, 3, 0, 6, 0, 3])
    counterexamples = [(np.array(
        [0 for _ in range(LENGTH)] + [-1 for _ in range(LENGTH)] + [0 for _ in range(2 * LENGTH + 1)] + [0, 15]))]
    iterations = 0
    while True:
        ...
        print(
            f'q+noise: {q + noise[1: 1 + LENGTH]} | dq+noise: {q + dq + noise[1: 1 + LENGTH]} | T+noise: {T + noise[0]} | N: {N}')
        optimizer = ps.single.GlobalBestPSO(n_particles=10000, dimensions=len(alignment), options=options,
                                            bounds=alignment_bounds, ftol=1, ftol_iter=50)  # bounds=bounds)
