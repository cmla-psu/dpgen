...
def unpack_inputs(all_inputs):
    ...
    noise = all_inputs[2 * LENGTH: 3 * LENGTH]
    return q, dq, noise
...
def convert_alignment_str(alignments):
    ...
    eta_dist_true = sp.simplify(f"{proof[2]} + {proof[3]} * dq_i + {proof[4]} * bq_aligned_dist")
    eta_dist_false = sp.simplify(f"{proof[5]} + {proof[6]} * dq_i + {proof[7]} * bq_aligned_dist")
    eta = sp.simplify(f"{holes[0]} + {holes[1]} * LENGTH")
    return f"selector: Omega ? {proof[0] == 0} : {proof[1] == 0}, eta: Omega ? {eta_dist_true} : {eta_dist_false}", f"eta: {eta}"
...
def noisy_max(all_inputs, alignments):
    ...
    while i < LENGTH:
        eta = noise[i]
        eta_dist = proof[2] + proof[3] * dq[i] + proof[4] * bq_aligned_dist if q[i] + eta > bq or i == 0 else proof[5] + \
                                                                                                              proof[6] * \
                                                                                                              dq[i] + \
                                                                                                              proof[
                                                                                                                  7] * bq_aligned_dist
        selector = proof[0] if q[i] + eta > bq or i == 0 else proof[1]
        cost = (cost if selector == 0 else 0) + np.abs(eta_dist) * (EPSILON / (holes[0] + holes[1] * LENGTH))
        if selector == 1:
            bq_aligned_dist = bq_shadow_dist
            imax_aligned_dist = imax_shadow_dist
        if q[i] + eta > bq or i == 0:
            failures += my_assert(q[i] + dq[i] + eta + eta_dist > bq + bq_aligned_dist or i == 0)
            ...
            bq = q[i] + eta
            bq_aligned_dist = dq[i] + eta_dist
        else:
            failures += my_assert(not (q[i] + dq[i] + eta + eta_dist > bq + bq_aligned_dist or i == 0))
    ...
    return cost, failures, (np.square((holes[0] + holes[1] * LENGTH) / EPSILON)) / (
        np.square((9 + 9 * LENGTH) / EPSILON))
...
def main():
    ...
    min_bounds = (np.array([-10 for _ in range(LENGTH)] + [-1 for _ in range(LENGTH)] + [-10 for _ in range(LENGTH)]))
    max_bounds = (np.array([10 for _ in range(LENGTH)] + [1 for _ in range(LENGTH)] + [10 for _ in range(LENGTH)]))
    bounds = (min_bounds, max_bounds)

    align_min_bounds = [0, 0] + [-5 for _ in range(6)] + [0, 0]
    align_max_bounds = [2, 2] + [5 for _ in range(6)] + [5, 10]
    alignment_bounds = (align_min_bounds, align_max_bounds)
    # q, dq, noise
    alignment = np.array([0 for _ in range(8)] + [1, 1])
