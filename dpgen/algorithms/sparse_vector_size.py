...
def convert_alignment_str(alignments):
    ...
    eta2 = sp.simplify(
        f"{holes[1]} + {holes[2]} * c + {holes[3]} * LENGTH"
    )
...
def svt(all_inputs, alignments):
    ...
    if holes[0] < 1e-5 or holes[1] + holes[2] * c + holes[3] * LENGTH < 1e-5:
    ...
    while i < LENGTH and count < c:
        if q[i] + noise[i + 1] >= T_bar:
            ...
            cost += np.abs(eta2_dist) * EPSILON / (holes[1] + holes[2] * c + holes[3] * LENGTH)
            # output True
            count += 1
        else:
            ...
            failures += my_assert(q[i] + dq[i] + noise[i + 1] + eta2_dist < T_bar + dist_T_bar)
            cost += np.abs(eta2_dist) * EPSILON / (holes[1] + holes[2] * c + holes[3] * LENGTH)
        # if cost > EPSILON - my_max(np.abs(eta2_dist) * EPSILON / (holes[1] + holes[2] * N), np.abs(eta2_dist) * EPSILON / (holes[1] + holes[2] * N)):
        # break
        i += 1
    ...
    return cost, failures, (
                np.square(holes[0] / EPSILON) + np.square((holes[1] + holes[2] * c + holes[3] * LENGTH) / EPSILON)) / (
                       np.square(9 / EPSILON) + np.square((9 + 9 * c + 9 * LENGTH) / EPSILON))
...
def find_alignments(alignments, all_inputs: np.ndarray):
    ...
    for i in numba.prange(alignments.shape[0]):
        ...
        if failures > 0:
            ...
            results[
                i] = variance  # - (count_false / LENGTH) + np.abs(count_true - N) / max(N, LENGTH - N)# - 20 * (count_true / LENGTH)
    return results
...
def main():
    ...
    alignment = np.array([0 for _ in range(7)] + [1, 1, 0, 0])

    align_min_bounds = [-5 for _ in range(7)] + [0, 0, 0, 0]
    align_max_bounds = [5 for _ in range(7)] + [10, 10, 10, 10]
    ...
    while True:
        ...
        optimizer = ps.single.GlobalBestPSO(n_particles=10000, dimensions=len(alignment), options=options,
                                            bounds=alignment_bounds, ftol=1, ftol_iter=100)  # bounds=bounds)
