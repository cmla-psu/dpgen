...
def find_alignments(alignments, all_inputs: np.ndarray):
    ...
    for i in numba.prange(alignments.shape[0]):
        ...
        # hard requirement
        if failures > 0:
            results[i] = 1000 * EPSILON * failures
        else:
            results[
...
def main():
    ...
    # alignment = np.array([1, 1, 0, -1, 0, 0, 0, 3, 0, 3])
    counterexamples = [
        (np.array([0 for _ in range(LENGTH)] + [-1 for _ in range(LENGTH)] + [0 for _ in range(LENGTH + 1)] + [0, 15]))]
    iterations = 0
    while True:
